using System;
using System.Collections.Generic;

namespace HttpD
{
    public class Encryption
    {
        private static readonly byte[] ShiftRowsTable = { 0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11 };
        private static readonly byte[] ShiftRowsTableInv = { 0, 13, 10, 7, 4, 1, 14, 11, 8, 5, 2, 15, 12, 9, 6, 3 };

        #region Lookup tables

        private static readonly byte[] LookupRcon =
        {
            0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a
        };

        private static readonly byte[] LookupSbox =
        {
            
        };

        private static readonly byte[] LookupSboxInv =
        {
            
        };

        private static readonly byte[] LookupG2 =
        {
            
        };

        private static readonly byte[] LookupG3 =
        {
            
        };

        private static readonly byte[] LookupG9 =
        {
            
        };

        private static readonly byte[] LookupG11 =
        {
            
        };

        private static readonly byte[] LookupG13 =
        {
            
        };

        private static readonly byte[] LookupG14 =
        {
            
        };


        #endregion

        /// <summary>
        /// Apply and reverse the rijndael s-box to all elements in an array
        /// http://en.wikipedia.org/wiki/Rijndael_S-box
        /// </summary>
        public static void SubBytes(byte[] a) {
            for (var i=0;i<a.Length;i++)
                a[i]=LookupSbox[a[i]];
        }
        public static void SubBytesInv(byte[] a) {
            for (var i = 0; i < a.Length; i++)
                a[i] = LookupSboxInv[a[i]];
        }

        /// <summary>
        /// Perform the core key schedule transform on 4 bytes, as part of the key expansion process
        /// http://en.wikipedia.org/wiki/AES_key_schedule#The_key_schedule
        /// </summary>
        public static void KeySchedule(byte[] a, int i) {
            var temp = a[0]; // Rotate the output eight bits to the left
            a[0] = a[1];
            a[1] = a[2];
            a[2] = a[3];
            a[3] = temp;
            SubBytes(a);      // Apply Rijndael's S-box on all four individual bytes in the output word
            a[0] ^= LookupRcon[i]; // On just the first (leftmost) byte of the output word, perform the rcon operation with i
            // as the input, and exclusive or the rcon output with the first byte of the output word
        }

        /// <summary>
        /// Xor's the elements in A, by B, starting at the given offset in B
        /// </summary>
        public static void Xor(byte[] a, List<byte> b, int offset) {
            for (var i = 0; i < a.Length; i++)
                a[i] ^= b[offset + i];
        }

        /// <summary>
        /// Expand the 128-bit key to 11 round keys
        /// http://en.wikipedia.org/wiki/AES_key_schedule#The_key_schedule
        /// </summary>
        public static byte[] Expand(byte[] key) {
            var keys = new List<byte>();
            keys.AddRange(key);       // The first 16 bytes of the expanded key are simply the encryption key
            var i=1;                  // The rcon iteration value i is set to 1
            var t=new byte[4];     // Temporary working area known as 't' in the Wiki article
            while (keys.Count<176) {  // Until we have 176 bytes of expanded key, we do the following:
                keys.CopyTo(keys.Count - 4, t, 0, 4); // We assign the value of the previous four bytes in the expanded key to t
                KeySchedule(t, i);                // We perform the key schedule core on t, with i as the rcon iteration value
                i++;                                  // We increment i by 1
                Xor(t,keys,keys.Count-16);            // We exclusive-or t with the four-byte block 16 bytes before the new expanded key. 
                keys.AddRange(t);                     // This becomes the next 4 bytes in the expanded key

                // We then do the following three times to create the next twelve bytes
                for (var j=0;j<3;j++) {
                    keys.CopyTo(keys.Count - 4, t, 0, 4); // We assign the value of the previous four bytes in the expanded key to t
                    Xor(t,keys,keys.Count-16);            // We exclusive-or t with the four-byte block 16 bytes before the new expanded key. 
                    keys.AddRange(t);                     // This becomes the next 4 bytes in the expanded key
                }
            }
            return keys.ToArray();
        }

        /// <summary>
        /// Xor the current cipher state by a specific round key
        /// </summary>
        public static void XorCipherWithRoundKey(byte[] cipher, byte[] keys, int round) {
            for (var i=0;i<16;i++) {
                cipher[i] ^= keys[i + round * 16];
            }
        }

        /// <summary>
        /// Apply the shift rows step on the 16 byte cipher state
        /// http://en.wikipedia.org/wiki/Advanced_Encryption_Standard#The_ShiftRows_step
        /// </summary>
        public static void ShiftRows(byte[] state) {
            var temp = new byte[16];
            Array.Copy(state,temp,16);
            for (var i=0;i<16;i++)
                state[i]=temp[ShiftRowsTable[i]];
        }

        /// <summary>
        /// Reverse the shift rows step on the 16 byte cipher state
        /// </summary>
        public static void ShiftRowsInv(byte[] state) {
            var temp = new byte[16];
            Array.Copy(state, temp, 16);
            for (var i = 0; i < 16; i++)
                state[i] = temp[ShiftRowsTableInv[i]];
        }

        /// <summary>
        /// Perform the mix columns matrix on one column of 4 bytes
        /// http://en.wikipedia.org/wiki/Rijndael_mix_columns
        /// </summary>
        public static void MixCol(byte[] cipher, int offset) {
            var a0 = cipher[offset+0];
            var a1 = cipher[offset+1];
            var a2 = cipher[offset+2];
            var a3 = cipher[offset+3];
            cipher[offset+0] = (byte)(LookupG2[a0] ^ LookupG3[a1] ^ a2 ^ a3);
            cipher[offset+1] = (byte)(LookupG2[a1] ^ LookupG3[a2] ^ a3 ^ a0);
            cipher[offset+2] = (byte)(LookupG2[a2] ^ LookupG3[a3] ^ a0 ^ a1);
            cipher[offset+3] = (byte)(LookupG2[a3] ^ LookupG3[a0] ^ a1 ^ a2);
        }

        /// <summary>
        /// Perform the inverse mix columns matrix on one column of 4 bytes
        /// </summary>
        public static void MixColInv(byte[] cipher, int offset) {
            var a0 = cipher[offset + 0];
            var a1 = cipher[offset + 1];
            var a2 = cipher[offset + 2];
            var a3 = cipher[offset + 3];
            cipher[offset + 0] = (byte)(LookupG14[a0] ^ LookupG9[a3] ^ LookupG13[a2] ^ LookupG11[a1]);
            cipher[offset + 1] = (byte)(LookupG14[a1] ^ LookupG9[a0] ^ LookupG13[a3] ^ LookupG11[a2]);
            cipher[offset + 2] = (byte)(LookupG14[a2] ^ LookupG9[a1] ^ LookupG13[a0] ^ LookupG11[a3]);
            cipher[offset + 3] = (byte)(LookupG14[a3] ^ LookupG9[a2] ^ LookupG13[a1] ^ LookupG11[a0]);
        }

        /// <summary>
        /// Perform the mix columns matrix on each column of the 16 bytes
        /// </summary>
        public static void MixCols (byte[] state) {
            MixCol(state, 0);
            MixCol(state, 4);
            MixCol(state, 8);
            MixCol(state, 12);
        }

        /// <summary>
        /// Perform the reverse mix columns matrix on each column of the 16 bytes
        /// </summary>
        public static void MixColsInv(byte[] state) {
            MixColInv(state, 0);
            MixColInv(state, 4);
            MixColInv(state, 8);
            MixColInv(state, 12);
        }

        /// <summary>
        /// Encrypt a single 128 bit block by a 128 bit key using AES 
        /// http://en.wikipedia.org/wiki/Advanced_Encryption_Standard
        /// </summary>
        public static byte[] Encrypt(byte[] m, byte[] k) {
            // Key expansion
            var keys = Expand(k);

            // First Round
            var c = new byte[16];
            Array.Copy(m,c,16);
            XorCipherWithRoundKey(c, keys, 0);

            // Middle rounds
            for (var i=0; i<9; i++) {
                SubBytes(c);
                ShiftRows(c);
                MixCols(c);
                XorCipherWithRoundKey(c, keys, i+1);
            }

            // Final Round
            SubBytes(c);
            ShiftRows(c);
            XorCipherWithRoundKey(c, keys, 10);

            return c;
        }

        /// <summary>
        /// Decrypt a single 128 bit block by a 128 bit key using AES 
        /// </summary>
        public static byte[] Decrypt(byte[] m, byte[] k) {
            // Key expansion
            var keys = Expand(k);

            // Reverse the final Round
            var c = new byte[16]; Array.Copy(m,c,16);
            XorCipherWithRoundKey(c,keys,10);
            ShiftRowsInv(c);
            SubBytesInv(c);

            // Reverse the middle rounds
            for (var i=0; i<9; i++) {
                XorCipherWithRoundKey(c,keys,9-i);
                MixColsInv(c);
                ShiftRowsInv(c);
                SubBytesInv(c);
            }

            // Reverse the first Round
            XorCipherWithRoundKey(c,keys,0);

            return c;
        }

        /// <summary>
        /// Convert a hex string to bytes
        /// </summary>
        public static byte[] ToBytes(string str) {
            var b = new byte[str.Length / 2];
            for (var i = 0; i < str.Length / 2; i++)
                b[i] = (byte)Convert.ToInt32(str.Substring(i*2,2), 16);
            return b;
        }

        /// <summary>
        /// Pretty-print a byte array
        /// </summary>
        public static void Pretty(string label, byte[] a)
        {
            var arr = "";
            foreach (var b in a)
            {
                if (arr.Length > 0) arr += "-";
                arr += b.ToString("X2");
            }
            
            Console.WriteLine("{0}:\n{1}", label, arr);
        }
    }
}
